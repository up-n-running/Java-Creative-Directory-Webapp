...
String REPORT_SQL = "SELECT whereDidYouHearRef ref, whereDidYouHearOther other, whereDidYouHearMagazine magazine, count(*) frequency " +
                    ...
                    "INNER JOIN MEMBERS m ON ( m.memberContactId = mc.memberContactId ) " +
                    "WHERE   m.lastPaymentDate IS NOT NULL " +
                    ( toDate   == null ? "" : "AND   m.regDate <= ? " ) +
                    ( fromDate == null ? "" : "AND   m.regDate >= ? " ) +
                    ( magName.length()   == 0 ? "" : "AND  ( UPPER( mc.whereDidYouHearMagazine ) LIKE ? " ) +
                    ( otherName.length() == 0 ? "" : ( magName.length() == 0 ? "AND ( " : "OR" ) + "  UPPER( mc.whereDidYouHearOther    ) LIKE ? " ) + ")" +
...
<form action="whereDidYouHear.jsp" method="post">
<input type="hidden" name="fromForm" value="true" />
<table border="0" cellpadding="0" cellspacing="0">
<tr>
  <td colspan="5" class="title">Customers Where Did You hear About Us Report</td>
</tr>
<tr>
  <td class="formLabel">Registered Between</td>
